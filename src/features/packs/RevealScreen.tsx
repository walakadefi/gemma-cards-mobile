...
import { AccessibilityInfo, Animated, Image, PanResponder, Pressable, StyleSheet, Text, Vibration, View } from 'react-native';
...
export function RevealScreen({ pack, onRip, onClose, onViewBinder, onBrowsePacks, existingCardNames = [] }: { pack: DemoPack; onRip: () => void; onClose: () => void; onViewBinder?: () => void; onBrowsePacks?: () => void; existingCardNames?: string[] }) {
  ...
  const knownCardNames = useMemo(() => new Set(existingCardNames), [existingCardNames]);
  const visibleCardCount = (state.visibleIndex ?? -1) + 1;
  ...
  return <AppScreen scroll><View style={styles.content}>
    ...
    {state.phase === 'sealed' || state.phase === 'tearing' ? <>
      <View style={[styles.starField, { pointerEvents: 'none' }]}>{stars.map(([left, top, size], index) => <View key={index} style={[styles.star, { left: `${left}%`, top: `${top}%`, width: size, height: size }]} />)}</View>
      ...
      <View style={styles.packStage}>
        {isOpening ? <Animated.View style={[styles.foilGlow, { pointerEvents: 'none', backgroundColor: presentation.accent, boxShadow: `0px 0px 44px ${presentation.accent}`, opacity: ripProgress.interpolate({ inputRange: [0, 0.3, 0.8, 1], outputRange: [0, 0.7, 0.45, 0] }), transform: [{ scale: ripProgress.interpolate({ inputRange: [0, 1], outputRange: [0.8, 1.35] }) }] }]} /> : null}
        {isOpening ? <Animated.View style={[styles.emergingCard, { pointerEvents: 'none', opacity: ripProgress.interpolate({ inputRange: [0, 0.45, 0.7, 1], outputRange: [0, 0, 1, 1] }), transform: [{ perspective: 900 }, { translateY: ripProgress.interpolate({ inputRange: [0, 0.45, 1], outputRange: [110, 110, -46] }) }, { scale: ripProgress.interpolate({ inputRange: [0, 0.45, 1], outputRange: [0.86, 0.86, 1] }) }, { rotateY: ripProgress.interpolate({ inputRange: [0, 0.68, 1], outputRange: ['0deg', '0deg', '360deg'] }) }] }]}><Text style={styles.emergingGem}>◆</Text></Animated.View> : null}
      ...
      <View style={styles.cardRevealStage}>
        {state.phase === 'browsing' ? <View style={[styles.cardStack, { pointerEvents: 'none' }]}><View style={[styles.stackCard, styles.stackCardBack]} /><View style={[styles.stackCard, styles.stackCardFront]} /></View> : null}
        {isRarePull ? <Animated.View style={[styles.hitHalo, { pointerEvents: 'none', opacity: hitPulse.interpolate({ inputRange: [1, 1.035], outputRange: [0.25, 0.7] }), transform: [{ scale: hitPulse.interpolate({ inputRange: [1, 1.035], outputRange: [0.95, 1.12] }) }] }]} /> : null}
