export type RipFlowState =
  ...
  | { phase: 'tearing'; visibleIndex: null }
  | { phase: 'browsing'; visibleIndex: number }
  | { phase: 'suspense'; visibleIndex: 8 }
  | { phase: 'complete'; visibleIndex: 9 };

export type RipFlowAction = { type: 'START_RIP' } | { type: 'RIP_OPENED' } | { type: 'NEXT' } | { type: 'SUSPENSE_FINISHED' };
...
export function ripFlowReducer(state: RipFlowState, action: RipFlowAction): RipFlowState {
  if (action.type === 'START_RIP' && state.phase === 'sealed') return { phase: 'tearing', visibleIndex: null };
  if (action.type === 'RIP_OPENED' && state.phase === 'tearing') return { phase: 'browsing', visibleIndex: 0 };
